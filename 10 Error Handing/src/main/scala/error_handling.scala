...
object error_handling {
  ...
  object PaymentCardValidator {
    ...
    def valideteDateCard(date: String): AllErrorsOr[String] = {
      ...
      def validateDateYearsMounts: AllErrorsOr[String] =
        date.split("\\s+").toList match {
          case a :: b :: Nil
              if (a.toIntOption >= Option(1) && a.toIntOption <= Option(12)
                && b.toIntOption >= Option(16) && b.toIntOption <= Option(
                21
              )) =>
            date.validNec
    ...
    def validateBankName(bankName: String): AllErrorsOr[String] =
      if (
        bankName == "BelarusBank" || bankName == "BPSBank" || bankName == "MTBBank"
      ) bankName.validNec
      else BankNameValide.invalidNec

    def validatePaymentSystem(paymentSystem: String): AllErrorsOr[String] =
      if (paymentSystem == "Viza" || paymentSystem == "MasterCard")
        paymentSystem.validNec
    ...
    def validate(
      ...
      (
        validateNameCardLength(name),
        validateNomberCard(number),
        valideteDateCard(expirationDate),
        validateSecurityCode(securityCode),
        validateBankName(bankName),
        validatePaymentSystem(paymantSystem)
      ).mapN(PaymentCard)
  }
}
