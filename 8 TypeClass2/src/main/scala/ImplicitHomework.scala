  ...
  object ImplicitsHomework {
    ...
    object SuperVipCollections4s {
      ...
      object syntax {
        implicit class GetSizeScoreOps[T: GetSizeScore](inner: T) {
          def sizeScore: SizeScore =
            implicitly[GetSizeScore[T]].apply(inner) //implement the syntax!
        }
      }
      ...
      final class MutableBoundedCache[K: GetSizeScore, V: GetSizeScore](
        ...
        def scopeSizeScore: SizeScore = {
          map.foldLeft[SizeScore](0) {
            case (acc, (x, y)) => acc + x.sizeScore + y.sizeScore
          }
        }

        def put(key: K, value: V): Unit = {
          if (
            map.nonEmpty && maxSizeScore < scopeSizeScore + key.sizeScore + value.sizeScore
          ) {
            val (k, _) = map.head
            map.remove(k)
            put(key, value)
      ...
      object instances {
        ...
        implicit val stringSizeScore: GetSizeScore[String] = (value: String) =>
          12 + value.length * 2
        ...
        implicit def mapSizeScore[K: GetSizeScore, V: GetSizeScore]
        ...
        implicit def packedMultyMapSizeScore[K: GetSizeScore, V: GetSizeScore]
      ...
      def createTwitCache(maxSizeScore: SizeScore): TwitCache =
        new TwitCache {
          ...
          override def put(twit: Twit): Unit =
            mutableBoundedCache.put(twit.id, twit)
